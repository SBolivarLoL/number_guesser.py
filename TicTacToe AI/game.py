import time
from player import Human_player, ComputadoraInteligente, RandomComputer_player

class TicTacToe:
    def __init__(self):
        # usamos solo una lista para representar el tablero 3x3
    ...
    def movimientos_disponi(self):
        # usamos list comprenhension para volver todo lo que está abajo en una sola linea
        # enumerate() crea una lista con tuplas adentro que en este caso tendran x u o y su indice
    ...
    def ganador(self, cuadro, letter):
        ...
        #verificar diagonales, pero solo los números pares que son las únicas diagonales viables
        if cuadro % 2 == 0:
...
if __name__ == '__main__':
    ...
    jugador_o = ComputadoraInteligente('O')
    #t es tic tac toe
    t = TicTacToe()
    jugar(t, jugador_x, jugador_o, print_juego=True)
    #para comprobar que la computadora inteligente nunca pierde podemos ponerla a jugar contra la random computer
    # y hacer iterar el juego 1000 veces con dos if y llevando una cuenta de las veces que gana cada letra y las 
    #veces que hay empates